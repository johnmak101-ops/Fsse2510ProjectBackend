...
public interface ProductRepository
    ...
    @Query("SELECT DISTINCT p FROM ProductEntity p LEFT JOIN FETCH p.images WHERE p.slug IN :slugs")

    List<ProductEntity> findAllBySlugIn(@Param("slugs") Collection<String> slugs);

    @Modifying

    @Query(value = "UPDATE product_inventory SET stock = stock - :quantity WHERE pid = :pid AND stock >= :quantity", nativeQuery = true)

    int deductStock(@Param("pid") Integer pid, @Param("quantity") Integer quantity);
    ...
    @Query("UPDATE ProductEntity p SET p.promotion = NULL WHERE p.promotion.id = :promoId")

    void removePromotionFromProducts(@Param("promoId") Integer promoId);
    ...
    @Query(value = """
            ...
            LEFT JOIN product_collection col ON p.collection_id = col.id

            WHERE (:category IS NULL OR cat.name = :category)

              AND (:collection IS NULL OR col.name = :collection)

              AND (:lastPid IS NULL OR p.pid < :lastPid)
    ...
    List<Integer> findPidsByCursor(

            @Param("category") String category,

            @Param("collection") String collection,

            @Param("lastPid") Integer lastPid,
    ...
    @Query("SELECT DISTINCT p FROM ProductEntity p LEFT JOIN FETCH p.inventories WHERE p.pid IN :pids")

    List<ProductEntity> findInventoriesByPidIn(@Param("pids") Collection<Integer> pids);

    @Query("SELECT DISTINCT p FROM ProductEntity p LEFT JOIN FETCH p.images WHERE p.pid IN :pids")

    List<ProductEntity> findImagesByPidIn(@Param("pids") Collection<Integer> pids);
